...
impl EventsListenerBuilder {
    ...
    pub fn add_clan(mut self, tag: &str) -> Self {
        self.event_type
            .push(EventType::Clan(tag.to_string(), Instant::now(), None));
        self
    }
    ...
    pub fn add_player(mut self, tag: &str) -> Self {
        self.event_type
            .push(EventType::Player(tag.to_string(), Instant::now(), None));
        self
    }
    ...
    pub fn add_war(mut self, tag: &str) -> Self {
        self.event_type
            .push(EventType::War(tag.to_string(), Instant::now(), None));
        self
    }
...
impl std::fmt::Display for EventsError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            self.event_type, self.api_error
        )
    }
}
        ...
        if let Some(duration) = duration {
            ...
            while start.elapsed() < duration {
                if let Err(e) = self.fire_events().await {
                    ...
                    self.handler
                        .on_error(e.api_error, e.tag, e.event_type)
                        .await;
                };
            }
        } else {
            ...
            match event {
                EventType::Player(tag, last_fired, old) => {
                    if let Some(duration) = Instant::now().checked_duration_since(*last_fired) {
                        if Self::should_fire_again(duration, 5) {
                ...
                EventType::War(tag, last_fired, old) => {
                    if let Some(duration) = Instant::now().checked_duration_since(*last_fired) {
                        if Self::should_fire_again(duration, 60 * 10) {
                            return match self.client.get_current_war(tag, false).await {
                                Ok(new) => {
