...
impl APIAccount {
    ...
    pub async fn login(credential: Credential, ip: &str) -> anyhow::Result<(Self, String)> {
        ...
        let login_response = client
            ...
            .await
            .context(format!(
                "login response failed to parse for {}",
                credential.email()
            ))?;
        ...
        account.get_keys(&client).await.context(format!(
        ...
        if account.keys.len() < 10 {
            ...
            for _ in 0..(10 - account.keys.len().min(10)) {
                account.create_key(&client, &ip).await.context(format!(
                    "failed to create key for {}",
                    account.credential.email()
                ))?;
            }
        }
        ...
        account
            ...
            .await
            .context(format!(
                "failed to update all keys for {}",
                account.credential.email()
            ))?;

        #[cfg(feature = "tracing")]
        tracing::debug!(
            "fetching {}'s keys (post update)",
            account.credential.email()
        );
        account.get_keys(&client).await.context(format!(
            "failed to get keys for {}",
    ...
    pub async fn re_login(&mut self) -> anyhow::Result<()> {
        ...
        let ip = self.response.temporary_api_token.limits[1]
            .cidrs
            .as_ref()
            .unwrap()[0]
            .clone();
    ...
    pub async fn update_all_keys(
        ...
        let tasks = bad_keys
            .iter()
            .map(|key| self.revoke_key(client, &key.id))
            .collect::<Vec<_>>();
        futures::future::join_all(tasks)
            .await
            .into_iter()
            .for_each(|maybe_key| match maybe_key {
                Ok(_) => {
