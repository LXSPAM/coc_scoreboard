...
impl Client {
    ...
    const IP_URL: &'static str = "https://api.ip.pe.kr";
    /// Returns a [`Client`]
    ///
    /// # Errors
    ///
    ...
    async fn init(&self, credentials: Credentials) -> anyhow::Result<()> {
        ...
        *self.ip_address.lock() = ip.clone();
        let tasks = credentials
            .0
            .into_iter()
            .map(|credential| dev::APIAccount::login(credential, &ip));
        let accounts = futures::future::join_all(tasks)
            .await
            .into_iter()
        ...
        for account in accounts {
            self.accounts
                .insert(account.0.credential.clone(), account.0);
        }
    ...
    ///
    /// This function will return an error if the request fails
    pub async fn get_current_war(
        &self,
        clan_tag: &str,
        is_cwl: bool,
    ) -> Result<war::War, APIError> {
        #[cfg(feature = "tracing")]
        ...
        if is_cwl {
            match self.get_league_group(&clan_tag).await {
                Ok(group) => return self.get_league_war_by_clan_tag(group, &clan_tag).await,
                Err(e) => return Err(e),
            }
        }
        let result: Result<war::War, APIError> = self.parse_json(self.get(url), false).await;

        match result {
            Ok(war) => {
                if war.state == "notInWar" {
                    match self.get_league_group(&clan_tag).await {
                        Ok(group) => self.get_league_war_by_clan_tag(group, &clan_tag).await,
                        Err(APIError::NotFound) => Ok(war),
                        Err(e) => return Err(e),
                    }
                } else {
                    Ok(war)
            ...
            Err(APIError::AccessDenied) => match self.get_league_group(&clan_tag).await {
                Ok(group) => self.get_league_war_by_clan_tag(group, &clan_tag).await,
                Err(e) => return Err(e),
            },
            Err(e) => return Err(e),
    ...
    pub async fn get_league_group(&self, clan_tag: &str) -> Result<war::LeagueGroup, APIError> {
        #[cfg(feature = "tracing")]
        tracing::trace!("get_league_group({})", clan_tag);
        let clan_tag = clan_tag.parse::<LogicLong>()?.to_string();
        let url = format!(
    ...
    pub async fn get_league_war(&self, war_tag: &str) -> Result<war::War, APIError> {
        #[cfg(feature = "tracing")]
        tracing::trace!("get_league_war({})", war_tag);
        let war_tag = war_tag.parse::<LogicLong>()?.to_string();
        let url = format!(
            "{}/clanwarleagues/wars/{}",
            Self::BASE_URL,
            urlencoding::encode(&war_tag)
        );
        self.parse_json(self.get(url), false).await
    }
    pub async fn get_league_war_by_clan_tag(
        &self,
        group: war::LeagueGroup,
        clan_tag: &str,
    ) -> Result<war::War, APIError> {
        let mut war_tags = Vec::new();
        'outer: for war in group.rounds {
            for tag in war.war_tags {
                if tag == "#0" {
                    break 'outer;
                }
                war_tags.push(tag);
        ...
        for war_tag in war_tags {
            let war = self.get_league_war(&war_tag).await?;
            if war.state == "preparation" || war.state == "inWar" {
                if war.clan.tag.clone() == clan_tag || war.opponent.tag.clone() == clan_tag {
                    if clan_tag != war.clan.tag {
                        let changed_war = war::War {
                            state: war.state.clone(),
